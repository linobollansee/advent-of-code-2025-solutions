...
long long solvePart2(const vector<Junction>& junctions) {
    ...
    for (const auto& pair : pairs) {
        if (uf.unionSets(pair.i, pair.j)) {
            ...
            // Check if all are connected (only one circuit)
            vector<int> circuits = uf.getCircuitSizes();
            if (circuits.size() == 1) {
                break;
            }
        }
    ...
    long long x1 = (long long)junctions[lastConnection.i].x;
    long long x2 = (long long)junctions[lastConnection.j].x;
    return x1 * x2;
}
...
int main() {
    ...
    cout << "C++ (Day 8 Part 2 - Simple)" << endl;
