...
def solve_part2(tiles):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            min_x = min(tiles[i][0], tiles[j][0])
            max_x = max(tiles[i][0], tiles[j][0])
            min_y = min(tiles[i][1], tiles[j][1])
            max_y = max(tiles[i][1], tiles[j][1])
...
print(f"Python (Day 9 Part 2 - Simple)")
