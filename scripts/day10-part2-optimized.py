...
def find_min_switch_flips(current, switches, parity_maps, cache):
    ...
    for pattern, n_flips in parity_maps[current_parity].items():
        if all(p <= c for p, c in zip(pattern, current)):
            next_state = [(c - p) // 2 for c, p in zip(current, pattern)]
            found, count = find_min_switch_flips(next_state, switches, parity_maps, cache)
...
print(f"Python (Day 10 Part 2 - Optimized)")
